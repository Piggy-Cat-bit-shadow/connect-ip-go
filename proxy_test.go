...
func TestProxyRuntimeStatsUsesServerQUICStream(t *testing.T) {
	...
	require.NotZero(t, stats.CongestionWindow, "%+v", stats)
	require.NotZero(t, stats.CurrentPMTU, "%+v", stats)
}
...
func TestTTLs(t *testing.T) {
	...
	t.Run("IPv6", func(t *testing.T) {
		...
		packetHopLimit1 := []byte{
			...
			0x3b, 0x01, // No Next Header, Hop Limit (1)
		...
		packet := []byte{
			0x60, 0x00, 0x00, 0x00, // Version, Traffic Class, Flow Label
			0x00, 0x01, // Payload Length
			0x3b, 0x2A, // No Next Header, Hop Limit (42)
			0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, // Source IP
			0x20, 0x01, 0x48, 0x60, 0x48, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0x88, // Destination IP
			0x00,
		}
